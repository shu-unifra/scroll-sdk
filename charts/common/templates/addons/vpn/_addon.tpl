...
{{- if .Values.addons.vpn.enabled -}}
  ...
  {{- if and .Values.addons.vpn.configFile (not .Values.addons.vpn.configFileSecret) }}
    {{- $secret := include "scroll.common.addon.vpn.secret" . -}}
    {{- if $secret -}}
  ...
  {{- $scriptVolume := include "scroll.common.addon.vpn.scriptsVolumeSpec" . | fromYaml -}}
  {{- if $scriptVolume -}}
  ...
  {{- $configVolume := include "scroll.common.addon.vpn.configVolumeSpec" . | fromYaml }}
  {{ if $configVolume -}}
  ...
  {{- $networkpolicy := include "scroll.common.addon.vpn.networkpolicy" . -}}
  {{- if $networkpolicy -}}
