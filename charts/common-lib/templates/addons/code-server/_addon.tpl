...
*/}}
{{- define "scroll.common.addon.codeserver" -}}
  {{- if .Values.addons.codeserver.enabled -}}
    {{/* Append the code-server container to the sidecars */}}
    {{- $container := include "scroll.common.addon.codeserver.container" . | fromYaml -}}
    {{- if $container -}}
    ...
    {{- if or .Values.addons.codeserver.git.deployKey .Values.addons.codeserver.git.deployKeyBase64 -}}
      {{- $deployKeySecret := include "scroll.common.addon.codeserver.deployKeySecret" . -}}
      {{- if $deployKeySecret -}}
    ...
    {{- if or .Values.addons.codeserver.git.deployKey .Values.addons.codeserver.git.deployKeyBase64 .Values.addons.codeserver.git.deployKeySecret }}
      {{- $volume := include "scroll.common.addon.codeserver.deployKeyVolumeSpec" . | fromYaml -}}
      {{- if $volume -}}
    ...
    {{- if .Values.addons.codeserver.service.enabled -}}
      ...
      {{- include "scroll.common.class.service" $ -}}
      {{- $_ := unset $.ObjectValues "service" -}}
    {{- end -}}

    {{/* Add the code-server ingress */}}
    {{- $svcName := printf "%v-addon-codeserver" (include "scroll.common.lib.chart.names.fullname" .) -}}
